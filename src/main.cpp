...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uHIC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/50.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
static const int64 nTargetTimespan = 1 * 5 * 60; // Retarget every Five Minutes
...
void ThreadScriptCheck() {
    RenameThread("hawaiicoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1393718240, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x7c1f9184 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x7c, 0x1f, 0x91, 0x84 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HawaiicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HawaiicoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("HawaiicoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running HawaiicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HawaiicoinMiner terminated\n");
        throw;
    }
}
